...
class GCond_recons:
    ...
    def __init__(self, data, args, device='cuda', **kwargs):
        ...
        self.Projectron_matrix=nn.Parameter(torch.FloatTensor(self.data.feat_full.shape[0], n).to(device))
        self.class_relation=np.zeros((self.data.nclass,self.data.nclass))

        self.reset_parameters()
        self.optimizer_proj = torch.optim.Adam([self.Projectron_matrix],lr=args.lr_proj)
        self.optimizer_feat = torch.optim.Adam([self.feat_syn], lr=args.lr_feat)
        self.optimizer_pge = torch.optim.Adam(self.pge.parameters(), lr=args.lr_adj)
        print('adj_syn:', (n,n), 'feat_syn:', self.feat_syn.shape)

    def reset_parameters(self):
        temp=torch.randn(self.Projectron_matrix.size())
        self.Projectron_matrix.data.copy_(temp)
        ...
        for it in range(args.epochs+1):
            ...
            for ol in range(outer_loop):
                ...
                for c in range(data.nclass):
                    ...
                    emb_reconstruction=torch.mm(self.Projectron_matrix,feat_syn)
                    output_reconstruction= model.forward_sampler(emb_reconstruction[n_id], adjs)
                    loss_reconstruct = F.nll_loss(output_reconstruction, labels[n_id[:batch_size]])
                    gw_reconstruct = torch.autograd.grad(loss_reconstruct, model_parameters,retain_graph=True)

                    matchloss2=match_loss(gw_reconstruct, gw_real, args, device=self.device)
                    P1 = torch.ones((self.Projectron_matrix.shape[0], 1)).to(self.device)
                    P2 = torch.ones((self.Projectron_matrix.shape[1], 1)).to(self.device)
                    pojec_regular_loss=F.mse_loss(torch.mm(self.Projectron_matrix,P2),P1)
                    ...
                    loss += coeff  * (match_loss(gw_syn, gw_real, args, device=self.device) + matchloss2 + pojec_regular_loss)
